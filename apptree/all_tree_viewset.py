...
class Tree_ALL_View(viewsets.ViewSet):
    ...
    def do_del_leafs(self, request):
        ...
        for tree in tree_qs:
            leaf_2_del_lisx = [({"id": ll.id, "name": ll.name, "noOfpaper": ll.noOfpaper})
                               for ll in tree.branchesfrk.branchleafgroupfrk.groupleafs.all()
                               # reverse using the related name groupleafs
            ...
            if len(leaf_2_del_lisx) > 0:  leaf_2_del_lis.extend(
                leaf_2_del_lisx)  # use extend and no need for list inside list

            # print (len(leaf_2_del_lis))
            # for l in tree.branchesfrk.branchleafgroupfrk.groupleafs.all() :
            #     if leaf_name == l.name :  print(l.name)
        if len(leaf_2_del_lis) == 0: return Response({"tree": treename, "leaf": leaf_name, "error": "leaf Not Found"},
    ...
    def do_upd_branch(self, request):
        ...
        branch_2_upd_lisx = [{"id": treex.branchesfrk.id, "name": treex.branchesfrk.name}
                             for treex in tree_qs.all() if
                             treex.branchesfrk.name == branchname]  ##forwrd using the FK branchesfrk
        ...
        for ix in branch_2_upd_lisx:
            id_upd = ix["id"]
            branch2upd = Branch.objects.filter(id=id_upd).update(name=upd_name,
                                                                 length=upd_length)  # more efficient: one SQL update statement, and is much faster
            # branch2upd.save()
            # branch2upd = Branch.objects.get(id=ix["id"])
            # branch2upd.length = upd_length
            # branch2upd.save()

        return Response({"tree": treename, "branch": upd_name, "length": upd_length},
                        status=status.HTTP_200_OK)  ### instance.delete
